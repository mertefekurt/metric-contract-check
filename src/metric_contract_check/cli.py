...
from metric_contract_check import __version__
from metric_contract_check.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from metric_contract_check.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='metric-contract-check',
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help='metric contract text or event spec',
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"metric-contract-check: error: {exc}", file=sys.stderr)
        return 1
